...
local eof = P(-1)
local newline = lpeg.Cmt((Pn + rn)  lpeg.Carg(1) ,count_lines)
local line_comment =   (1 - newline) ^0  (newline + eof)
local blank = S t + newline + line_comment
local blank0 = blank ^ 0
local blanks = blank ^ 1
local alpha = Raz + RAZ + _
local alnum = alpha + R09
local str_c = (1 - S()) + P()  1
...
local id = R09 ^ 1  tonumber + max  Cc(-1)
...
local prefix_field = Prequired  CcLABEL_REQUIRED +
	Poptional  CcLABEL_OPTIONAL +
	Prepeated  CcLABEL_REPEATED
...
function dealoption(v)
	self.options = self.options or {}
	self.options[v.name] = v.value
end

function dealextend(v)
